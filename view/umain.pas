...
begin
  ...
  try
    ...
    begin
      ...
      // Busca os endereços na base
      if FEndereco.EnderecoExistente(QryTemp, LLogradouro, LCidade, LEstado) then
      begin
        if MessageDlg('Foram encontrados na base de dados, mais de um registro para o endereço informado. Deseja visualiza-los?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
        begin
          // Abre o formulário de seleção de CEPs, mas agora com os endereços do banco
          FrmSelecionaCep := TFrmSelecionaCep.Create(nil);
          try
            FrmSelecionaCep.CarregarEnderecosDoBanco(QryTemp, QryEnderecos, LLogradouro, LCidade, LEstado); // Método que carrega os dados da Query no Grid
